...
public class EventReader {
	 ...
	 * @throws IOException
	 */
	public CalendarEvent readEvent() throws IOException
	{
		String line;
		line = br.readLine();
		...
		if (values.length == 2)
		{
			...
			if (detailsPartTwo.length > 3)
			{
				...
				//create event
				e = new CalendarEvent(title, stringDate, detailsPartOne[4], endingTime);
			}
			//else if title is one word
			else	
				e = new CalendarEvent(detailsPartTwo[2], stringDate, detailsPartOne[4], detailsPartTwo[1]);
		...
		else if (values.length == 1)
		{
			...
			if (details.length > 6)
			{
				...
				e = new CalendarEvent(title, stringDate, startingTime);
			}
			else //title is only one word
				e = new CalendarEvent(details[5], stringDate, startingTime);
		}
